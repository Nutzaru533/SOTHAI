codeunit 60007 "INT_GenerateBarcode_SNY"
{
    procedure GenerateBarcode(var Barcode: Record INT_Barcode_SNY)
    begin
        DoGenerateBarcode(Barcode);
    end;

    local procedure DoGenerateBarcode(var Barcode: Record INT_Barcode_SNY)
    var
        Arguments: Record INT_RESTWebService_SNY temporary;
    begin
    ...
    local procedure InitArguments(var Arguments: Record INT_RESTWebService_SNY temporary; Barcode: Record INT_Barcode_SNY)
    var
        BaseURL: Text;
    ...
    local procedure CallWebService(var Arguments: Record INT_RESTWebService_SNY temporary) Success: Boolean
    var
        RESTWebService: codeunit INT_RESTWebServiceCode_SNY;
    begin
        Success := RESTWebService.CallRESTWebService(Arguments);
    end;

    local procedure SaveResult(var Arguments: Record INT_RESTWebService_SNY temporary; var Barcode: Record INT_Barcode_SNY)
    var
    ...
    begin
        FieldRec.Get(Database::INT_Barcode_SNY, fieldno);
