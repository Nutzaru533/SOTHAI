page 60007 "INT_SalesOrderOdata_SNY"
{
    Caption = 'Web Service Sales Order';
    PageType = Document;
    ...
    layout
    {
        area(content)
        {
            group(General)
            {
                ...
                group("INT_Sony_SNY")
                {
                    Caption = 'Marketplace Parameters';

                    field("INT_MarketPlace_SNY"; INT_MarketPlace_SNY)
                    {
                        ApplicationArea = all;

                    }
                    field("INT_SellerID_SNY";
                    INT_SellerID_SNY)
                    {
                        ApplicationArea = all;
                        Editable = false;
                    ...
                    field("INT_Channel_SNY"; INT_Channel_SNY)
                    {
                        ApplicationArea = All;
                    }

                    field("INT_MktplaceOrderID_SNY"; INT_MktplaceOrderID_SNY)
                    {
                        ApplicationArea = All;
                    }

                    field("INT_OrderType_SNY"; INT_OrderType_SNY)
                    {
                        ApplicationArea = All;
                    }

                    field("INT_MktSyncStatus_SNY"; INT_MktSyncStatus_SNY)
                    {
                        ApplicationArea = All;
                        Editable = False;
                    }
                    field("INT_MktOrdStatus_SNY"; INT_MktOrdStatus_SNY)
                    {
                        ApplicationArea = All;
                        Editable = false;
                    }
                    field("INT_OrderStatus_SNY"; INT_OrderStatus_SNY)
                    {
                        ApplicationArea = All;
                        Editable = false;
                    }

                    field("INT_InvCheck_SNY"; INT_InvCheck_SNY)
                    {
                        ApplicationArea = All;
                        Editable = false;
                    }
                    field("INT_SimpleStatus_SNY"; INT_SimpleStatus_SNY)
                    {
                        ApplicationArea = All;
                        trigger OnValidate()
                        begin
                            if GuiAllowed then begin
                                if INT_SimpleStatus_SNY <> xrec.INT_SimpleStatus_SNY then
                                    Error('Simple Status cannot be changed manually!');
                            end;
                        end;
                    }

                    field("INT_SimpleRemarks_SNY"; INT_SimpleRemarks_SNY)
                    {
                        ApplicationArea = All;

                    }

                    field("INT_DeliveryType_SNY"; INT_DeliveryType_SNY)
                    {
                        ApplicationArea = All;
                    }

                    field("INT_DelConfirmed_SNY"; INT_DelConfirmed_SNY)
                    {
                        ApplicationArea = All;
                        Editable = false;
                    }
                    field("INT_SAPOrderID_SNY"; INT_SAPOrderID_SNY)
                    {
                        ApplicationArea = All;
                    }
                    field("INT_SAPInvoiceID_SNY"; INT_SAPInvoiceID_SNY)
                    {
                        ApplicationArea = All;
                    }

                    field("INT_Remarks1_SNY"; INT_Remarks1_SNY)
                    {
                        ApplicationArea = All;
                    }
                    field("INT_Remarks2_SNY"; INT_Remarks2_SNY)
                    {
                        ApplicationArea = All;
                    }
                    field("INT_Remarks3_SNY"; INT_Remarks3_SNY)
                    {
                        ApplicationArea = All;
                    }
                    field("INT_InternalProcessing_SNY"; INT_InternalProcessing_SNY)
                    {
                        ApplicationArea = All;
                        Editable = false;
                    }
                    field("INT_BCOrderNo_SNY"; INT_BCOrderNo_SNY)
                    {
                        ApplicationArea = All;
                        Editable = false;
                    }
                    field("INT_ProcessErr_SNY"; INT_ProcessErr_SNY)
                    {
                        ApplicationArea = All;
                    }

                    group("INT_Shopify_SNY")
                    {
                        Caption = 'Additional Parameters';

                        field(INT_FinanicalStatus_SNY; INT_FinanicalStatus_SNY)
                        {
                            ApplicationArea = All;
                        }

                        field(INT_ShippingProfile_SNY; INT_ShippingProfile_SNY)
                        {
                            ApplicationArea = All;
                        }

                        field(INT_RiskScore_SNY; INT_RiskScore_SNY)
                        {
                            ApplicationArea = All;
                        }

                        field(INT_RiskRecommendation_SNY; INT_RiskRecommendation_SNY)
                        {
                            ApplicationArea = All;
                        }

                        field(INT_EmailSendToCustomer_SNY; INT_EmailSendToCustomer_SNY)
                        {
                            ApplicationArea = All;
                        }
                        field(INT_TrackingURL_SNY; INT_TrackingURL_SNY)
                        {
                            ApplicationArea = All;
                            Visible = false;
                        }
                        field(INT_TrackingField_SNY; INT_TrackingField_SNY)
                        {
                            ApplicationArea = All;
                            Visible = false;
                        }
                        ...
                        field("Shopify Shipping Agent Code"; "Shipping Agent Code")
                        {
                            ApplicationArea = All;
                        }

                        field("Shopify Package Tracking No."; "Package Tracking No.")
                        {
                            ApplicationArea = All;
                        }
                        field("Shopify Payment Method"; "Shopify Payment Method")
                        {
                            ApplicationArea = All;
                        }
                        field("Fullfillment Remarks"; "Fullfillment Remarks")
                        {
                            ApplicationArea = All;
                        }

                        field("Discount Target"; "Discount_Target")
                        {
                            ApplicationArea = All;
                        }
                        ...
                        field("Seller Voucher Amount"; "Seller Voucher Amount")
                        {
                            ApplicationArea = all;
                        }
                        field("Marketplace Voucher Amount"; "Marketplace Voucher Amount")
                        {
                            ApplicationArea = all;
                        }
                        field("Order File To WH"; "Order File To WH")
                        {
                            ApplicationArea = all;
                        }
                        field("Consolidated File to WH"; "Consolidated File to WH")
                        {
                            ApplicationArea = all;
                        }
                    }
